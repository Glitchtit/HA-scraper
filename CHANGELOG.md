...
- Add-on: resolved `s6-overlay-suexec: fatal: can only run as pid 1` error by migrating from `CMD ["/run.sh"]` to proper s6-overlay service directories (`/etc/services.d/`).
