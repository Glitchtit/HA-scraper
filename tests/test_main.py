...
class TestEnvInt:
    ...
    def test_returns_int(self):
        from addon.main import _env_int
...
class TestSyncProduct:
    ...
    def test_creates_new_product(self):
        ...
        storage.add_barcode.assert_called_once_with(99, "999")

    def test_skips_existing_barcode_from_storage_api(self):
        storage = self._storage()
        storage.get_product_by_barcode.return_value = {"id": 5, "name": "Existing"}
    ...
    def test_create_product_error_returns_false(self):
        storage = self._storage()
        storage.create_product.side_effect = StorageAPIError("fail")
...
class TestImageExtension:
    def test_known_mime_types(self):
        from addon.main import _image_extension
...
class TestMainDryRun:
    ...
    def test_dry_run_prints_products(self, capsys):
        ...
        with patch("addon.main.KRuokaScraper") as MockScraper:
...
class TestParseArgsAIFlags:
    ...
    def test_gemini_model_default(self):
        from addon.main import _GEMINI_DEFAULT_MODEL
        args = parse_args([
            "--sort",
            "--storage-url", "https://storage.example.com",
            "--gemini-api-key", "KEY",
...
class TestValidateArgsAI:
    ...
    def test_valid_sort_passes(self):
        from addon.main import _validate_args
...
class TestCallGemini:
    @patch("addon.main.requests.post")
    def test_returns_text_from_response(self, mock_post):
        from addon.main import _call_gemini
        mock_resp = MagicMock()
...
class TestCallGeminiJson:
    @patch("addon.main.time.sleep")
    @patch("addon.main._call_gemini")
    def test_returns_parsed_json(self, mock_gemini, _mock_sleep):
        from addon.main import _call_gemini_json
...
class TestAiSortProducts:
    def _make_storage(self, products, locations):
        g = MagicMock(spec=StorageClient)
    ...
    def test_updates_products_with_ai_locations(self, mock_gemini):
        from addon.main import _ai_sort_products
        products = [{"id": 1, "name": "Maito"}, {"id": 2, "name": "Pesuaine"}]
        locations = [{"id": 2, "name": "Fridge"}, {"id": 3, "name": "Cleaning Cabinet"}]
        storage = self._make_storage(products, locations)
        mock_gemini.return_value = '{"1": 2, "2": 3}'

        result = _ai_sort_products(storage, "gemini-key")
        assert result == 2
        storage.update_product.assert_any_call(1, location_id=2)
        storage.update_product.assert_any_call(2, location_id=3)
    ...
    def test_transfers_stock_to_new_location(self, mock_gemini):
        ...
        storage = self._make_storage(products, locations)
        mock_gemini.return_value = '{"1": 2}'
        storage.get_product_stock_locations.return_value = [
            {"location_id": 5, "amount": 3.0},
        ]

        result = _ai_sort_products(storage, "gemini-key")
        assert result == 1
        storage.transfer_stock.assert_called_once_with(1, 3.0, 5, 2)
    ...
    def test_skips_transfer_when_stock_already_at_target(self, mock_gemini):
        ...
        _ai_sort_products(storage, "gemini-key")
        storage.transfer_stock.assert_not_called()
    ...
    def test_transfers_from_multiple_locations(self, mock_gemini):
        ...
        _ai_sort_products(storage, "gemini-key")
        assert storage.transfer_stock.call_count == 2
        storage.transfer_stock.assert_any_call(1, 2.0, 5, 2)
        storage.transfer_stock.assert_any_call(1, 1.0, 6, 2)
    ...
    def test_transfer_error_does_not_abort(self, mock_gemini):
        ...
        storage.get_product_stock_locations.side_effect = [
            [{"location_id": 5, "amount": 1.0}],
            [{"location_id": 2, "amount": 1.0}],
        ]
        storage.transfer_stock.side_effect = [StorageAPIError("fail"), None]
    ...
    def test_no_locations_returns_zero(self, mock_gemini):
        ...
        storage.get_locations.return_value = []
        result = _ai_sort_products(storage, "key")
        assert result == 0
    ...
    def test_no_products_returns_zero(self, mock_gemini):
        from addon.main import _ai_sort_products
        storage = MagicMock(spec=StorageClient)
        storage.get_locations.return_value = [{"id": 2, "name": "Fridge"}]
        storage.get_all_products.return_value = []
        result = _ai_sort_products(storage, "key")
        assert result == 0
        mock_gemini.assert_not_called()
    ...
    def test_invalid_json_skips_batch(self, mock_gemini, _mock_sleep):
        ...
        result = _ai_sort_products(storage, "key")
        assert result == 0
        storage.update_product.assert_not_called()

    @patch("addon.main._call_gemini")
    def test_storage_error_on_locations_returns_zero(self, mock_gemini):
        from addon.main import _ai_sort_products
        storage = MagicMock(spec=StorageClient)
        storage.get_locations.side_effect = StorageAPIError("fail")
        result = _ai_sort_products(storage, "key")
        assert result == 0
        mock_gemini.assert_not_called()
...
class TestAiAssignDueDates:
    def _make_storage(self, products):
        g = MagicMock(spec=StorageClient)
        g.get_all_products.return_value = products
        g.update_product.return_value = None
        return g
    ...
    def test_updates_products_with_ai_dates(self, mock_gemini):
        from addon.main import _ai_assign_due_dates
        products = [{"id": 1, "name": "Maito"}, {"id": 5, "name": "Pasta"}]
        storage = self._make_storage(products)
        mock_gemini.return_value = '{"1": 14, "5": 1095}'

        result = _ai_assign_due_dates(storage, "gemini-key")
        assert result == 2
        storage.update_product.assert_any_call(1, default_best_before_days=14)
        storage.update_product.assert_any_call(5, default_best_before_days=1095)
    ...
    def test_no_products_returns_zero(self, mock_gemini):
        ...
        storage = MagicMock(spec=StorageClient)
        storage.get_all_products.return_value = []
        result = _ai_assign_due_dates(storage, "key")
        assert result == 0
        mock_gemini.assert_not_called()
    ...
    @patch("addon.main._call_gemini")
    def test_storage_error_returns_zero(self, mock_gemini):
        from addon.main import _ai_assign_due_dates
        storage = MagicMock(spec=StorageClient)
        storage.get_all_products.side_effect = StorageAPIError("fail")
        result = _ai_assign_due_dates(storage, "key")
        assert result == 0
        mock_gemini.assert_not_called()
...
class TestDeduplicateParentProducts:
    ...
    def test_merges_synonym_parents(self, mock_gemini):
        ...
        from addon.main import _deduplicate_parent_products
        products = [
            {"id": 10, "name": "Mausteet"},
            {"id": 11, "name": "Mauste"},
    ...
    def test_redirect_map_used_by_group(self, mock_gemini):
        ...
        from addon.main import _ai_group_products
        products = [
            {"id": 10, "name": "Makeiset"},
            {"id": 100, "name": "Curry", "parent_id": 10},
        ...
        with patch(
            "addon.main._deduplicate_parent_products",
    ...
    @patch("addon.main._fix_broken_product_units", return_value=0)
    @patch("addon.main._ai_detect_package_sizes", return_value=0)
    @patch("addon.main._ensure_units_and_conversions", return_value={"piece": 1})
    @patch("addon.main._call_gemini_json")
    def test_incremental_optimize_skips_dedup(self, mock_gemini_json, _m_ens, _m_pkg, _m_fix):
        ...
        from addon.main import _ai_optimize_products
        products = [
            {"id": 10, "name": "Makeiset",
             "active": False},
            {"id": 100, "name": "Curry", "parent_id": 10},
...
@patch("addon.main._deduplicate_parent_products", return_value=(0, {}))
class TestAiGroupProducts:
    ...
    def test_groups_products_under_new_parent(self, mock_gemini, _mock_dedup):
        ...
        result = _ai_group_products(storage, "gemini-key")
        assert result == 2
        # Dedup NOT called in full mode.
        _mock_dedup.assert_not_called()
        # "Group master" and category product groups should be ensured.
        storage.ensure_product_group.assert_any_call("Group master")
        storage.ensure_product_group.assert_any_call("Maitotaloustuotteet")
        ...
        storage.update_product.assert_any_call(
            100,
    ...
    def test_full_mode_strips_parents_and_sends_all(self, mock_gemini, _mock_dedup):
        ...
        storage.get_all_products.side_effect = [
            products,
        ...
        storage.update_product.assert_any_call(1, parent_id=None)
        ...
        storage.delete_product.assert_any_call(99)
    ...
    def test_no_products_returns_zero(self, mock_gemini, _mock_dedup):
        ...
        storage = MagicMock(spec=StorageClient)
        storage.get_all_products.return_value = []
        result = _ai_group_products(storage, "key")
        assert result == 0
        mock_gemini.assert_not_called()
    ...
    @patch("addon.main._call_gemini")
    def test_storage_error_returns_zero(self, mock_gemini, _mock_dedup):
        from addon.main import _ai_group_products
        storage = MagicMock(spec=StorageClient)
        storage.get_all_products.side_effect = StorageAPIError("fail")
        result = _ai_group_products(storage, "key")
    ...
    def test_does_not_set_product_as_own_parent(self, mock_gemini, _mock_dedup):
        ...
        calls = [c for c in storage.update_product.call_args_list
    ...
    def test_passes_location_and_quantity_unit(self, mock_gemini, _mock_dedup):
        ...
        _ai_group_products(storage, "key", location_id=5, quantity_unit_id=3)
        storage.create_product.assert_called_once_with(
    ...
    def test_group_without_product_group_on_ensure_failure(self, mock_gemini, _mock_dedup):
        ...
        storage = self._make_storage(products)
        storage.ensure_product_group.side_effect = StorageAPIError("fail")
        mock_gemini.return_value = (
            '{"1": {"parent": "Maito", "category": "Maitotaloustuotteet"}, '
            '"2": {"parent": "Maito", "category": "Maitotaloustuotteet"}}'
        )
        ...
        update_calls = [c for c in storage.update_product.call_args_list if c[0] == (100,)]
        assert len(update_calls) == 0, f"Unexpected parent update: {update_calls}"
        # Children should be updated without product_group_id.
        storage.update_product.assert_any_call(1, parent_id=100)
        storage.update_product.assert_any_call(2, parent_id=100)
    ...
    def test_full_mode_skips_parent_for_min_stock_product(self, mock_gemini, _mock_dedup):
        ...
        calls = [str(c) for c in storage.update_product.call_args_list]
        parent_calls_for_1 = [c for c in storage.update_product.call_args_list
        ...
        storage.update_product.assert_any_call(1, product_group_id=self._CATEGORY_GROUP_ID)
        # Product 2 gets parent normally.
        storage.update_product.assert_any_call(2, parent_id=100, product_group_id=self._CATEGORY_GROUP_ID)
    ...
    def test_incremental_mode_uses_dedup(self, mock_gemini, _mock_dedup):
        ...
        result = _ai_group_products(storage, "gemini-key", product_ids=[1])
        assert result == 1
    ...
    def test_incremental_mode_skips_already_grouped(self, mock_gemini, _mock_dedup):
        ...
        result = _ai_group_products(storage, "gemini-key", product_ids=[1, 2])
        assert result == 1
    ...
    def test_incremental_mode_includes_existing_hints(self, mock_gemini, _mock_dedup):
        ...
        _ai_group_products(storage, "gemini-key", product_ids=[20])
        prompt_text = mock_gemini.call_args[0][0]
    ...
    def test_full_mode_uses_optimize_model(self, mock_gemini, _mock_dedup):
        ...
        _ai_group_products(
            storage, "gemini-key", "gemini-1.5-flash",
...
class TestMainAIMode:
    @patch("addon.main._ai_sort_products")
    @patch("addon.main.StorageClient")
    def test_sort_mode_calls_ai_sort(self, MockStorage, mock_sort):
    ...
    @patch("addon.main._ai_assign_due_dates")
    @patch("addon.main.StorageClient")
    def test_date_mode_calls_ai_dates(self, MockStorage, mock_date):
    ...
    @patch("addon.main._ai_sort_products")
    @patch("addon.main.StorageClient")
    def test_sort_and_date_together(self, MockStorage, mock_sort, mock_date):
    ...
    @patch("addon.main._ai_group_products")
    @patch("addon.main.StorageClient")
    def test_group_mode_calls_ai_group(self, MockStorage, mock_group):
...
class TestDiscoverChainsAI:
    ...
    @patch("addon.main._ai_optimize_products")
    @patch("addon.main._discover_products", return_value=(0, [42, 99]))
    @patch("addon.main.StorageClient")
    def test_discover_chains_optimize(
        self, MockStorage, mock_discover, mock_optimize,
    ):
    ...
    @patch("addon.main._ai_optimize_products")
    @patch("addon.main._discover_products", return_value=(0, [42]))
    ...
    @patch("addon.main._ai_optimize_products")
    @patch("addon.main._discover_products", return_value=(1, []))
    ...
    @patch("addon.main._ai_optimize_products")
    @patch("addon.main._discover_products", return_value=(0, []))
    @patch("addon.main.StorageClient")
...
class TestDeleteAllProducts:
    @patch("addon.main.StorageClient")
    def test_empty_database_returns_0(self, MockStorage):
        from addon.main import _delete_all_products
        storage = MockStorage.return_value
        storage.get_all_products.return_value = []
        assert _delete_all_products(storage) == 0
        storage.delete_product.assert_not_called()

    @patch("addon.main.StorageClient")
    def test_deletes_all_products(self, MockStorage):
        from addon.main import _delete_all_products
        storage = MockStorage.return_value
        storage.get_all_products.return_value = [
            {"id": 1, "name": "Milk"},
        ...
        assert _delete_all_products(storage) == 0
        assert storage.delete_product.call_count == 3
        storage.delete_product.assert_any_call(1)
        storage.delete_product.assert_any_call(2)
        storage.delete_product.assert_any_call(3)

    @patch("addon.main.StorageClient")
    def test_fetch_error_returns_1(self, MockStorage):
        from addon.main import _delete_all_products
        storage = MockStorage.return_value
        storage.get_all_products.side_effect = StorageAPIError("connection refused")
        assert _delete_all_products(storage) == 1

    @patch("addon.main.StorageClient")
    def test_partial_failure_returns_1(self, MockStorage):
        from addon.main import _delete_all_products
        storage = MockStorage.return_value
        storage.get_all_products.return_value = [
            {"id": 1, "name": "Milk"},
        ...
        storage.delete_product.side_effect = [None, StorageAPIError("failed")]
        assert _delete_all_products(storage) == 1
        assert storage.delete_product.call_count == 2
...
class TestDiscoverProducts:
    ...
    @patch("addon.main.StorageClient")
    def test_no_pending_returns_0(self, MockStorage, MockScraper):
        storage_instance = MockStorage.return_value
        storage_instance.get_all_barcodes.return_value = []
        storage_instance.get_barcode_queue.return_value = []

        from addon.main import _discover_products
        from argparse import Namespace
        args = Namespace(
            storage_url="https://storage.example.com",
            store="N110",
    ...
    @patch("addon.main.KRuokaScraper")
    @patch("addon.main.StorageClient")
    def test_unknown_barcode_searched_on_kruoka(self, MockStorage, MockScraper):
        from addon.main import _discover_products
        from argparse import Namespace
        ...
        storage_instance.get_product_by_barcode.side_effect = [
            None,  # sync_product check
            {"id": 99, "name": "Maito"},  # post-creation lookup
        ]
        storage_instance.create_product.return_value = 99
        ...
        storage_instance.create_product.assert_called_once()
        storage_instance.add_stock.assert_called_once_with(99, amount=1.0)
        storage_instance.update_barcode_queue_item.assert_called_once_with(
    ...
    @patch("addon.main.KRuokaScraper")
    @patch("addon.main.StorageClient")
    def test_not_found_on_kruoka_or_skaupat_skips(self, MockStorage, MockScraper, mock_sk):
        from addon.main import _discover_products
        from argparse import Namespace
        ...
        assert rc == 0
        assert discovered_ids == []
        storage_instance.create_product.assert_not_called()
        storage_instance.update_barcode_queue_item.assert_called_once_with(
            42, status="error",
    ...
    @patch("addon.main.KRuokaScraper")
    @patch("addon.main.StorageClient")
    def test_skaupat_fallback_creates_product(self, MockStorage, MockScraper, mock_sk):
        ...
        storage_instance.create_product.return_value = 88
        ...
        storage_instance.create_product.assert_called_once()
        call_args = storage_instance.create_product.call_args
        assert call_args[1]["name"] == "Kotimaista luomukananmunat M6"
        storage_instance.add_stock.assert_called_once_with(88, amount=1.0)
        storage_instance.update_barcode_queue_item.assert_called_once_with(
    ...
    @patch("addon.main.KRuokaScraper")
    @patch("addon.main.StorageClient")
    def test_queue_fetch_error_returns_1(self, MockStorage, MockScraper):
        from scraper.storage_client import StorageAPIError
        from addon.main import _discover_products
        from argparse import Namespace

        MockStorage.return_value.get_all_barcodes.return_value = []
        MockStorage.return_value.get_barcode_queue.side_effect = StorageAPIError("fail")
...
class TestMultiStoreFallbackRunScraper:
    ...
    def test_fallback_to_second_store_on_error(self, capsys):
        ...
        with patch("addon.main.KRuokaScraper", side_effect=make_scraper):
            rc = main(["--store", "N110,N137", "--browse", "--dry-run"])
...
class TestMultiStoreDiscoverFallback:
    ...
    def test_discover_tries_second_store(
        ...
        storage_instance.get_product_by_barcode.return_value = {"id": 1}
...
class TestMainOptimizeMode:
    @patch("addon.main._ai_optimize_products")
    @patch("addon.main.StorageClient")
    def test_optimize_mode_calls_ai_optimize(self, MockStorage, mock_optimize):
...
@patch("addon.main._ai_detect_package_sizes", return_value=0)
@patch("addon.main._ensure_units_and_conversions", return_value={"piece": 1})
@patch("addon.main._optimize_units", return_value=0)
@patch("addon.main._deduplicate_parent_products", return_value=(0, {}))
class TestAiOptimizeProducts:
    ...
    def test_sort_date_group_in_single_pass(self, mock_gemini, _mock_dedup, _mock_opt, _mock_ens, _mock_pkg, _mock_fix_broken):
        ...
        result = _ai_optimize_products(storage, "gemini-key")
        # Dedup NOT called in full mode.
        _mock_dedup.assert_not_called()
        ...
        storage.update_product.assert_any_call(1, default_best_before_days=14)
        storage.update_product.assert_any_call(2, default_best_before_days=1095)
        # Check group: product 1 should be grouped under the new parent
        storage.create_product.assert_called()
        storage.update_product.assert_any_call(
    ...
    def test_pack_detection_sets_barcode_pack_info(self, mock_gemini, _mock_dedup, _mock_opt, _mock_ens, _mock_pkg, _mock_fix_broken):
        ...
        locations = [{"id": 2, "name": "Fridge"}]
        storage = self._make_storage(products, locations)
        storage.get_quantity_units.return_value = [
            {"id": 5, "name": "Piece", "abbreviation": "kpl"},
        ]
        storage.get_product_barcodes.return_value = [
        ...
        storage.update_barcode.assert_any_call(10, pack_size=4, pack_unit_id=5)
    ...
    def test_full_mode_strips_parents_and_sends_all(self, mock_gemini, _mock_dedup, _mock_opt, _mock_ens, _mock_pkg, _mock_fix_broken):
        ...
        _ai_optimize_products(storage, "gemini-key")
        # Parents should be stripped.
        storage.update_product.assert_any_call(11, parent_id=None)
        storage.update_product.assert_any_call(12, parent_id=None)
    ...
    def test_full_mode_deletes_old_parent_placeholders(self, mock_gemini, _mock_dedup, _mock_opt, _mock_ens, _mock_pkg, _mock_fix_broken):
        ...
        result = _ai_optimize_products(storage, "gemini-key")
        storage.delete_product.assert_any_call(20)
        assert result >= 1
    ...
    def test_full_mode_skips_parent_for_min_stock_product(self, mock_gemini, _mock_dedup, _mock_opt, _mock_ens, _mock_pkg, _mock_fix_broken):
        ...
        _ai_optimize_products(storage, "gemini-key")
        # Product 1 should NOT get parent_id (min_stock > 0)
        calls = storage.update_product.call_args_list
        parent_calls_for_1 = [c for c in calls
                              if c[0][0] == 1 and "parent_id" in (c[1] if len(c) > 1 else {})]
        assert len(parent_calls_for_1) == 0, f"Product 1 should not get parent: {calls}"
        # Product 1 SHOULD still get product_group_id.
        storage.update_product.assert_any_call(1, product_group_id=100)
        # Product 2 gets parent normally.
        storage.update_product.assert_any_call(
    ...
    def test_product_ids_filter(self, mock_gemini, _mock_dedup, _mock_opt, _mock_ens, _mock_pkg, _mock_fix_broken):
        ...
        result = _ai_optimize_products(storage, "gemini-key", product_ids=[1])
        assert result >= 1
    ...
    def test_incremental_existing_parents_in_prompt(self, mock_gemini, _mock_dedup, _mock_opt, _mock_ens, _mock_pkg, _mock_fix_broken):
        ...
        _ai_optimize_products(storage, "gemini-key", product_ids=[12])
        prompt = mock_gemini.call_args[0][0]
    ...
    def test_cleanup_empty_parent_after_optimize(self, mock_gemini, _mock_dedup, _mock_opt, _mock_ens, _mock_pkg, _mock_fix_broken):
        ...
        _ai_optimize_products(storage, "gemini-key")
        storage.delete_product.assert_any_call(50)
    ...
    def test_incremental_pack_sets_barcode_pack_info(
        ...
        _ai_optimize_products(storage, "gemini-key", product_ids=[2])
    ...
    def test_pack_handling_still_applies_sort_date_group(
        ...
        _ai_optimize_products(storage, "gemini-key", product_ids=[2])
        # Pack info set on barcode
        storage.update_barcode.assert_any_call(10, pack_size=10, pack_unit_id=5)
        # Sort/date/group still applied to the SAME product
        storage.update_product.assert_any_call(2, location_id=3)
        storage.update_product.assert_any_call(2, default_best_before_days=28)


class TestCanonicalUnit:
    def test_standard_abbreviations(self):
        from addon.main import _canonical_unit
...
class TestEnsureUnitsAndConversions:
    def _make_storage(self, existing_units=None, existing_conversions=None):
        storage = MagicMock(spec=StorageClient)
        storage.get_quantity_units.return_value = existing_units or []
        storage.get_quantity_unit_conversions.return_value = existing_conversions or []
        storage.create_quantity_unit.side_effect = lambda name, *a, **kw: (
            100 + len([c for c in storage.create_quantity_unit.call_args_list])
        )
        storage.create_quantity_unit_conversion.return_value = 200
        return storage

    def test_creates_missing_units(self):
        from addon.main import _ensure_units_and_conversions
        storage = self._make_storage()
        result = _ensure_units_and_conversions(storage)
        # Should have created all 9 standard units
        assert storage.create_quantity_unit.call_count == len(
    ...
    def test_skips_existing_units_by_abbreviation(self):
        ...
        storage = self._make_storage(existing_units=existing)
        result = _ensure_units_and_conversions(storage)
        assert result["g"] == 5
        # Should not have created "g" again
        create_names = [c.args[0] for c in storage.create_quantity_unit.call_args_list]
    ...
    def test_creates_global_conversions(self):
        from addon.main import _ensure_units_and_conversions, _GLOBAL_CONVERSIONS
        storage = self._make_storage()
        _ensure_units_and_conversions(storage)
        assert storage.create_quantity_unit_conversion.call_count == len(_GLOBAL_CONVERSIONS)
    ...
    def test_skips_existing_conversions(self):
        ...
        storage = self._make_storage(existing_units=existing_units, existing_conversions=existing_convs)
        _ensure_units_and_conversions(storage)
        # The kg→g conversion should be skipped
        for call in storage.create_quantity_unit_conversion.call_args_list:
...
class TestAiDetectPackageSizes:
    def _make_storage(self, products=None, conversions=None):
        storage = MagicMock(spec=StorageClient)
        storage.get_quantity_unit_conversions.return_value = conversions or []
        storage.create_quantity_unit_conversion.return_value = 100
        return storage

    def test_creates_conversion_for_product_with_size(self, mock_gemini):
        from addon.main import _ai_detect_package_sizes
        mock_gemini.return_value = [
        ...
        storage = self._make_storage(products)
        count = _ai_detect_package_sizes(storage, products, abbrev, "key", "model")
        assert count == 1
        storage.create_quantity_unit_conversion.assert_called_once_with(
    ...
    def test_skips_products_with_existing_conversions(self, mock_gemini):
        ...
        storage = self._make_storage(products, conversions=existing_convs)
...
class TestAiDetectDensityConversions:
    def _make_storage(self, conversions=None):
        storage = MagicMock(spec=StorageClient)
        storage.get_quantity_unit_conversions.return_value = conversions or []
        storage.create_quantity_unit_conversion.return_value = 100
        return storage
    ...
    def test_creates_density_for_weight_only_product(self, mock_gemini):
        from addon.main import _ai_detect_density_conversions
        mock_gemini.return_value = [
        ...
        storage = self._make_storage(conversions=conversions)
        count = _ai_detect_density_conversions(storage, products, abbrev, "key", "model")
        # 1 primary (kg→l) + 5 derived (kg→dl, kg→ml, g→l, g→dl, g→ml)
        assert count == 6
        # Primary conversion is the first call
        storage.create_quantity_unit_conversion.assert_any_call(
...
class TestDeriveDensityConversions:
    def test_kg_to_l_derives_all_pairs(self):
        from addon.main import _derive_density_conversions
...
@patch("addon.main._fix_recipe_units", return_value=0)
@patch("addon.main._ai_detect_density_conversions", return_value=0)
@patch("addon.main._ai_detect_package_sizes", return_value=0)
@patch("addon.main._fix_broken_product_units", return_value=0)
@patch("addon.main._ensure_units_and_conversions")
class TestOptimizeUnits:
    def test_calls_pipeline_steps(self, mock_ensure, mock_fix_broken,
                                   mock_pkg, mock_density,
                                   mock_fix_recipes):
        from addon.main import _optimize_units
        storage = MagicMock(spec=StorageClient)
        storage.get_all_products.return_value = [{"id": 1, "name": "Maito"}]
        mock_ensure.return_value = {"g": 1, "l": 2}

        _optimize_units(storage, "key", "model")

        mock_ensure.assert_called_once_with(storage)
    ...
    def test_stops_on_ensure_failure(self, mock_ensure, mock_fix_broken,
        ...
        result = _optimize_units(storage, "key", "model")
        assert result == 0
...
class TestFixBrokenProductUnits:
    def _make_storage(self, units, products, conversions=None):
        storage = MagicMock(spec=StorageClient)
        storage.get_quantity_units.return_value = units
        storage.get_all_products.return_value = products
        storage.get_quantity_unit_conversions.return_value = conversions or []
        return storage

    def test_fixes_orphaned_weight_product(self):
        from addon.main import _fix_broken_product_units
        units = [{"id": 5, "name": "Gramma", "abbreviation": "g"}]
        products = [
            {"id": 1, "name": "Vehnäjauho 2kg", "unit_id": 999},
        ]
        storage = self._make_storage(units, products)
        abbrev = {"g": 5, "kg": 6, "kpl": 10}
        fixed = _fix_broken_product_units(storage, abbrev)
        assert fixed == 1
        call_kwargs = storage.update_product.call_args[1]
    ...
    def test_handles_update_failure(self):
        ...
        storage = self._make_storage(units, products)
        storage.update_product.side_effect = StorageAPIError("fail")
        abbrev = {"kpl": 10}
        fixed = _fix_broken_product_units(storage, abbrev)
        assert fixed == 0
    ...
    def test_cleans_orphaned_conversions(self):
        ...
        storage = self._make_storage(units, products, conversions)
        abbrev = {"kpl": 5}
        _fix_broken_product_units(storage, abbrev)
        # Should delete the 2 orphaned conversions
        assert storage.delete_quantity_unit_conversion.call_count == 2
        deleted_ids = {
            call.args[0]
            for call in storage.delete_quantity_unit_conversion.call_args_list
        }
    ...
    def test_fixes_stocked_product_directly(self):
        ...
        fixed = _fix_broken_product_units(storage, abbrev)
        assert fixed == 1
        storage.update_product.assert_called_once_with(1, unit_id=10)
    ...
    def test_fixes_product_with_volume_unit(self):
        ...
        fixed = _fix_broken_product_units(storage, abbrev)
        assert fixed == 1
        storage.update_product.assert_called_once_with(50, unit_id=17)
...
class TestFixRecipeUnits:
    def _make_storage(self, positions, products, units, conversions=None):
        storage = MagicMock(spec=StorageClient)
        storage.get_recipe_positions.return_value = positions
        storage.get_all_products.return_value = products
        storage.get_quantity_units.return_value = units
        storage.get_quantity_unit_conversions.return_value = conversions or []
        return storage
    ...
    def test_fixes_invalid_qu_id(self):
        from addon.main import _fix_recipe_units
        positions = [
        ...
        units = [{"id": 5, "name": "Kappale", "abbreviation": "kpl"}]
        storage = self._make_storage(positions, products, units)
        abbrev = {"kpl": 5}
        fixed = _fix_recipe_units(storage, abbrev)
        assert fixed == 1
        storage.update_recipe_position.assert_called_once_with(1, unit_id=5)
    ...
    def test_same_unit_is_noop(self):
        ...
        fixed = _fix_recipe_units(storage, abbrev)
        assert fixed == 0
        storage.update_recipe_position.assert_not_called()
    ...
    def test_skips_when_conversion_exists(self):
        ...
        storage = self._make_storage(positions, products, units, conversions)
        abbrev = {"kpl": 5, "g": 7}
        fixed = _fix_recipe_units(storage, abbrev)
        assert fixed == 0
        storage.update_recipe_position.assert_not_called()
    ...
    def test_empty_positions_is_noop(self):
        from addon.main import _fix_recipe_units
        storage = MagicMock(spec=StorageClient)
        storage.get_recipe_positions.return_value = []
        abbrev = {"kpl": 5}
        fixed = _fix_recipe_units(storage, abbrev)
        assert fixed == 0
...
class TestCheckRecipesForUnitGaps:
    ...
    def test_detects_cross_domain_gap(self):
        ...
        from addon.main import _check_recipes_for_unit_gaps
        positions = [
        ...
        with patch("addon.main._ai_detect_density_conversions", return_value=3) as mock_density:
            result = _check_recipes_for_unit_gaps(storage, {10}, abbrev, "key", "model")
...
class TestFixRecipeUnitsWithDensity:
    ...
    def test_attempts_density_for_cross_domain_gap(self):
        ...
        storage.get_quantity_unit_conversions.side_effect = [
            [],  # first call — no conversions
            refresh_conversions(),  # second call after density creation
        ]

        with patch("addon.main._ai_detect_density_conversions", return_value=2):
            fixed = _fix_recipe_units(storage, abbrev, "gemini_key", "model")
    ...
    def test_falls_back_when_density_fails(self):
        ...
        with patch("addon.main._ai_detect_density_conversions", return_value=0):
            fixed = _fix_recipe_units(storage, abbrev, "gemini_key", "model")

        # Density failed — should fall back to stock QU
        assert fixed == 1
        storage.update_recipe_position.assert_called_once_with(1, unit_id=8)
    ...
    def test_density_for_weight_recipe_volume_product(self):
        ...
        with patch("addon.main._ai_detect_density_conversions", return_value=2) as mock_d:
            ...
            fixed = _fix_recipe_units(storage, abbrev, "key", "model")
...
class TestIncrementalDensityAndRecipeCheck:
    ...
    @patch("addon.main._check_recipes_for_unit_gaps")
    @patch("addon.main._ai_detect_density_conversions")
    @patch("addon.main._ai_detect_package_sizes")
    @patch("addon.main._fix_broken_product_units", return_value=0)
    @patch("addon.main._ensure_units_and_conversions")
    def test_incremental_calls_density_and_recipe_check(
        ...
        storage.get_all_barcodes.return_value = []
        storage.get_product_groups.return_value = []
        storage.add_stock.return_value = None

        # product_ids=[10] triggers incremental mode (not full)
        result = _ai_optimize_products(
            storage, "gemini_key", "model",
            product_ids=[10],
        )
