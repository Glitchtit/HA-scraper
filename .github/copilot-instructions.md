...
1. **HA Supervisor Add-on** (`addon/`) — Docker container with s6-overlay running two services: the periodic scraper and an ingress web server (`ingress_server.py` on port 8099). Entry point: `addon/main.py`.
2. **HA Custom Integration** (`custom_components/scraper/`) — Sidebar panel + config flow + WebSocket API.
...
| Module | Role |
|---|---|
| `scraper/scraper.py` | K-ruoka.fi scraper (GraphQL + kr-api REST backends) |
| `scraper/storage_client.py` | HA-Storage REST API client |
| `scraper/skaupat_client.py` | S-kaupat.fi EAN lookup |
| `scraper/searxng_client.py` | SearXNG search client |
| `addon/main.py` | Entry point — argparse, Gemini AI helpers, `--sort`/`--date`/`--group`/`--optimize` |
| `addon/ingress_server.py` | HTTP server for HA ingress web UI |
| `custom_components/scraper/ws_api.py` | WebSocket API for HA sidebar panel |
...
The `scraper/` package is **copied identically** into `addon/scraper/`. Changes to `scraper.py`, `storage_client.py`, `skaupat_client.py`, or `searxng_client.py` must be applied to both locations.

### Gemini AI integration

All Gemini calls go through `_call_gemini()` → `_call_gemini_json()` in `addon/main.py`. Retries up to `_GEMINI_MAX_RETRIES` with exponential back-off. Sanitizes control characters from responses. Batch sizes: 100 for sort/date/group, 1000 for optimize.
...
- `StorageAPIError` (alias: `StorageAPIError`) is the exception for Storage API and Gemini failures (defined in `storage_client.py`).
...
| File | Field |
|---|---|
| `addon/config.yaml` | `version: "X.Y.Z"` |
| `custom_components/scraper/manifest.json` | `"version": "X.Y.Z"` |
| `addon/CHANGELOG.md` | New `## X.Y.Z` section |
...
- External API errors are wrapped in `StorageAPIError` rather than leaking `requests` exceptions.
